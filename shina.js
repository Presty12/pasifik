...
client.on("guildMemberAdd", member => {
member.setNickname('İsim | Yaş') // tagınızı başa ekleyebilirsiniz.
});
...
**Sunucumuza kayıt olmak için tagımızı alıp ses teyit odalarından birine geçmen yeterlidir. Seninle ${teyit}
...
${emoji2} Hesap katılım sırası : \`${number}/${number}\``))
...
let tag = (ayarlar.tag)
let sunucu = client.guilds.cache.get(ayarlar.sunucuid);
let kayıtkanal = sunucu.channels.cache.get(ayarlar.taglog);
let kayitSorumlusu = sunucu.roles.cache.get(ayarlar.tagges);
...
Date.prototype.toTurkishFormatDate = function (format) {
  ...
  client.on('ready', async () => {
    ...
    let channelID = client.channels.cache.get(ayarlar.kayıtkanal);
    let channel = guild.channels.cache.get(channelID);
    if(!channel) return console.log('Kanal bulunamadı.');
    const messages = [`<@&kayıtsızrolid> The Sunrise ses kanallarına giriş sağlayarak kaydınızı yaptırabilirsiniz. İyi eğlenceler <:meuw:797729959607009320>`];
